...
export class FourmPageComponent implements OnInit {
  ...
  constructor(private forumService : RestServiceService,private router : Router,private toastr : ToastrService){
  }
  ...
  doSearchPost(search : string)
  {
    ...
    if(this.posts.length==0)
    {
      ...
      this.toastr.error('No Results Found,Showing all posts')
    }
  }
    ...
    this.forumService.postQuestion(post_question).subscribe( (data : Question) => {
      if(data!=null)
      {
       ...
       this.toastr.success('Question posted successfully')
        ...
        this.toastr.error('Question not Posted')
      }
    });
  }
