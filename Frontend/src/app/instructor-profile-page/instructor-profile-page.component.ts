...
export class InstructorProfilePageComponent implements OnInit {
  ...
  constructor(config: NgbModalConfig, private modalService: NgbModal,private router : Router, private iProfileService : RestServiceService,private toastr : ToastrService) {
                      ...
                      test : string, video : string, image : string, price : string)
  {
    ...
    this.iProfileService.updateCourseDetails(post_course).subscribe( (data : Course) => {
      if(data!=null) {
       ...
       this.toastr.error("Course Details Not Updated");
      }
    });
