...
export class UserProfilePageComponent implements OnInit {
  ...
  constructor(config: NgbModalConfig, rconfig: NgbRatingConfig, private modalService: NgbModal, private profileService : RestServiceService, private router : Router, private activatedRoute : ActivatedRoute,private toastr : ToastrService) {
  ...
  submitProfileChanges(fullname : string, username : string, email : string, mobile : string, parent_mobile : string)
  {
    ...
    this.profileService.updateProfile(post_user).subscribe((data : User) => {
      if(data!=null) {
       ...
       this.toastr.error('Profile Details Not Updated')
      }
    });
  ...
  changeSuggestion(choice : string)
  {
    ...
    this.profileService.updateSuggesstion(post_user).subscribe((data : User) => {
      if(data!=null) {
       ...
       this.toastr.success("Personalized interest updated successfully")
      }
      else {
       // alert("Personalized interest not updated");
       this.toastr.error("Personalized interest not updated")
      }
    });
  }
  ...
  deleteAccount()
  {
    ...
    this.profileService.deleteUser(this.user.username).subscribe( (data:string) => { this.toastr.show(data) });
